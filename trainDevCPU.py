...
from data import load_file, preprocess_data, create_data_loader, preProcessing
...
def validate(model, criterion, epoch, epochs, iteration, iterations, data_loader_valid, save_path, train_loss,
    ...
    for inputs, labels in tqdm(data_loader_valid, total=len(data_loader_valid)):
        with torch.no_grad():
            ...
            y_pred = output.argmax(dim=1).data.numpy().flatten()
            y_true = labels.data.numpy().flatten()
...
if __name__ == '__main__':
    ...
    trainSet_01 = 'DataScriber/toyTrain_01.txt'
    validSet_01 = 'DataScriber/toyValid_01.txt'
    testSet_01 = 'DataScriber/toyTest_01.txt'
    # from sentences to list of words+punctuation
    preProcessing(trainSet_01)
    preProcessing(validSet_01)
    preProcessing(testSet_01)
    ...
    bert_punc = BertPunc(vocabSize, segment_size, output_size, dropout)
